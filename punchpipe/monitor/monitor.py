...
def _process_level(start_date, end_date, level):
    ...
    flow_query = (
        ...
        .where(
            or_(
                and_(Flow.start_time > start_date, Flow.end_time < end_date, Flow.flow_level == level),
                and_(Flow.flow_level == level, Flow.state == "running", Flow.start_time > start_date),
            )
        )
        .statement
    )
    file_query = (
        session.query(File)
        .where(and_(File.date_obs > start_date, File.date_obs < end_date, File.level == level))
        .statement
    )
    ...
    previous_flow_query = (
        ...
        .where(
            or_(
                and_(
                    Flow.start_time > previous_interval_start,
                    Flow.end_time < previous_interval_end,
                    Flow.flow_level == level,
    ...
    if len(flow_df):
        ...
        duration_plot = px.histogram(
            completed, x="duration", labels={"duration": "duration [sec]"}, title="Histogram of completed flow duration"
        )
        delay_plot = px.histogram(
            completed, x="delay", labels={"delay": "delay [sec]"}, title="Histogram of completed flow delay to start"
        )
        blocks = [
            f"## Level {level}: {start_date.strftime('%Y/%m/%d %H:%M')} to {end_date.strftime('%Y/%m/%d %H:%M')}",
            f"Ran on {datetime.now().strftime('%Y/%m/%d %H:%M')}",
            dp.Group(
                dp.BigNumber(
                    heading="Average Duration [sec]",
                    value=f"{average_duration:.1f}",
                    prev_value=f"{previous_average_duration:.1f}",
                ),
                dp.BigNumber(
                    heading="Stddev Duration [sec]",
            ...
            dp.Select(blocks=[dp.Plot(duration_plot, label="Duration plot"), dp.Plot(delay_plot, label="Delay plot")]),
...
def _file_inquiry(file_id):
    ...
    try:
        ...
        # Provenance
        children_ids = [
            e.child for e in session.query(FileRelationship).where(FileRelationship.parent == file_id).all()
        ]
...
def level_overview_page():
    return dp.View(
        dp.Form(
            on_submit=_process_level,
            controls=dp.Controls(
                start_date=dp.DateTime(label="Start datetime", initial=datetime.now() - timedelta(days=1)),
...
def file_inquiry_page():
    return dp.View(
        dp.Form(on_submit=_file_inquiry, controls=dp.Controls(file_id=dp.NumberBox(label="File ID", initial=0)))
    )


def flow_inquiry_page():
    return dp.View(
        dp.Form(on_submit=_flow_inquiry, controls=dp.Controls(flow_id=dp.NumberBox(label="Flow ID", initial=0)))
    )
