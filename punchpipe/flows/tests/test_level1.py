...
def test_level1_construct_file_info():
    level0_file = File(level=0,
                       ...
                       software_version='none',
                       date_obs=datetime.now())
    constructed_file_info = level1_construct_file_info.fn(level0_file)
    assert constructed_file_info.level == 1
    assert constructed_file_info.file_type == level0_file.file_type
    assert constructed_file_info.observatory == level0_file.observatory
    assert constructed_file_info.file_version == "0"
    assert constructed_file_info.software_version == __version__
    assert constructed_file_info.date_obs == level0_file.date_obs
    assert constructed_file_info.polarization == level0_file.polarization
...
def test_level1_construct_flow_info():
    ...
    level0_file = File(level=0,
                       ...
                       software_version='none',
                       date_obs=datetime.now())
    level1_file = level1_construct_file_info.fn(level0_file)
