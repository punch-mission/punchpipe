...
from punchbowl.quickpunch import quickpunch_core_flow
...
@task
def quickpunch_query_ready_files(session, pipeline_config: dict):
...
@task
def quickpunch_construct_flow_info(level1_files: File, level2_file: File, pipeline_config: dict):
...
@task
def quickpunch_construct_file_info(level1_files: t.List[File], pipeline_config: dict) -> t.List[File]:
...
@flow
def quickpunch_scheduler_flow(pipeline_config_path="config.yaml", session=None):
    generic_scheduler_flow_logic(
        quickpunch_query_ready_files,
        quickpunch_construct_file_info,
        quickpunch_construct_flow_info,
...
@flow
def quickpunch_process_flow(flow_id: int, pipeline_config_path="config.yaml", session=None):
    generic_process_flow_logic(flow_id, quickpunch_core_flow, pipeline_config_path, session=session)
