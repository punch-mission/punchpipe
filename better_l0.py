...
from punchbowl.data import NormalizedMetadata, get_base_file_name, write_ndcube_to_fits, write_ndcube_to_quicklook
...
from punchpipe.level0.core import interpolate_value
...
def get_metadata(db_classes, image_packets, image_shape, session) -> Tuple[Dict[str, Any], Dict[str, Any]]:
    acquisition_settings  = unpack_acquisition_settings(image_packets[0].SCI_XFI_HDR_ACQ_SET)

    observation_time = image_packets[0].timestamp
    spacecraft_id = image_packets[0].SCI_XFI_HDR_SCID
    ...
    before_pfw = (session.query(db_classes['ENG_PFW'])
    ...
    fits_info['COM_SET'] = image_packets[0].SCI_XFI_HDR_COM_SET
    fits_info['ACQ_SET'] = image_packets[0].SCI_XFI_HDR_ACQ_SET
    fits_info['DATE-OBS'] = image_packets[0].timestamp.isoformat()
...
@flow
def level0_core_flow(pipeline_config_path: str):
    logger = get_run_logger()
...
if __name__ == "__main__":
    config_path = "/Users/mhughes/repos/punchpipe/process_local_config.yaml"
    level0_core_flow(config_path)
    ...
    #
    # paths = sorted(glob(path_to_process + "*_09?_*.tlm"))
    # for tlm_index, path in enumerate(paths):
    #     print(tlm_index, len(paths), path)
    #     tlm_db_entry = TLMFile(
    #         path=path,
    #         successful=False,
    #         num_attempts=0,
    ...
    #                 contents = {key: parsed[packet_name][key][packet_num] for key in parsed[packet_name].keys()}
    #                 contents['index'] = packet_num
    #                 pkts.append(database_classes[packet_name](**contents))
    #             session.bulk_save_objects(pkts)
    #             session.commit()
    #         except (ValueError, sqlalchemy.exc.DataError):
    #             success = False
